...
export async function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("mecha.server.executeCommand", async () => {
...
function startDebugging(): Promise<void> {
    ...
    setTimeout(async () => {
        await vscode.debug.startDebugging(vscode.workspace.workspaceFolders[0], "mecha: Debug Server")
    }, 2000)
}
